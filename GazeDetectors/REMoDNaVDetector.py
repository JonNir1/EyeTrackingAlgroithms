...
class REMoDNaVDetector(BaseDetector):
    ...
    def _detect_impl(self, t: np.ndarray, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        classifier = remodnav.EyegazeClassifier(
            ...
            min_saccade_duration=cnfg.EVENT_MAPPING[cnst.EVENTS.SACCADE]["min_duration"] / cnst.MILLISECONDS_PER_SECOND,
            min_pursuit_duration=cnfg.EVENT_MAPPING[cnst.EVENTS.SMOOTH_PURSUIT]["min_duration"] / cnst.MILLISECONDS_PER_SECOND,
            min_fixation_duration=cnfg.EVENT_MAPPING[cnst.EVENTS.FIXATION]["min_duration"] / cnst.MILLISECONDS_PER_SECOND,
            max_pso_duration=cnfg.EVENT_MAPPING[cnst.EVENTS.PSO]["max_duration"] / cnst.MILLISECONDS_PER_SECOND,
        ...
        pp = classifier.preproc(xy,
                                min_blink_duration=cnfg.EVENT_MAPPING[cnst.EVENTS.BLINK]["min_duration"] / cnst.MILLISECONDS_PER_SECOND,
    ...
    def _parse_events(self, events: List[Dict[str, float]]) -> np.ndarray:
        events_map = {'FIXA': cnst.EVENTS.FIXATION,
                      'SACC': cnst.EVENTS.SACCADE,
                      'ISAC': cnst.EVENTS.SACCADE,
                      'HPSO': cnst.EVENTS.PSO,
                      'IHPS': cnst.EVENTS.PSO,
                      'LPSO': cnst.EVENTS.PSO,
                      'ILPS': cnst.EVENTS.PSO,
                      'PURS': cnst.EVENTS.SMOOTH_PURSUIT,
                      'BLNK': cnst.EVENTS.BLINK}
        candidates = np.full_like(self._candidates, cnst.EVENTS.UNDEFINED)
