...
import peyes.Analysis.figures as figs
...
class BasePipeline(ABC):
    ...
    def match_events(
        ...
        from peyes.Analysis.Calculators.EventMatchesCalculator import EventMatchesCalculator
        start = time.time()
        if verbose:
    ...
    def process_samples(
        ...
        """
        from peyes.Analysis.Calculators.SampleMetricsCalculator import SampleMetricsCalculator
        start = time.time()
    ...
    def process_event_features(
        ...
        """
        from peyes.Analysis.Calculators.EventFeaturesCalculator import EventFeaturesCalculator
        start = time.time()
        label_name = cnst.EVENT if label is None else label.name.lower()
        if verbose:
    ...
    def process_match_ratios(
        ...
        """
        from peyes.Analysis.Calculators.MatchRatioCalculator import MatchRatioCalculator
        start = time.time()
    ...
    def process_matched_features(
        ...
        from peyes.Analysis.Calculators.MatchedFeaturesCalculator import MatchedFeaturesCalculator
        start = time.time()
        event_name = cnst.EVENT if label is None else label.name.lower()
        if verbose:
    ...
    def __get_default_sample_metrics(label: Optional[cnfg.EVENT_LABELS]) -> Set[str]:
        ...
        transition_metrics = {
            "n-DKL",
            # "Transition Matrix KL-Divergence", "Transition Matrix l2-norm"  # uninteresting metrics
        }
        if label is None:
